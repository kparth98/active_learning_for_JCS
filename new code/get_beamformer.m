function w_norm = get_beamformer(N_ant, PT, f_start, f_end, H_comm, P_comm)
    loc_tx = 0:N_ant-1;
    delta_f = 2/N_ant;
    f_gap = f_end-f_start;
    f1 = -1 + delta_f/2;
    f2 = 1 - delta_f/2;
    w_base = cfirpm(N_ant-1,[f1,f1+f_gap,f1+f_gap+delta_f,f2],[1,1,0,0]).';
        
    w = w_base.*exp(1j*pi*loc_tx'*(f_start-f1));
